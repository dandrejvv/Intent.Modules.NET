...
- Breaking Change: `WolverineRegistrationFactoryExtension` no longer owns the `builder.Host.UseWolverine(...)` registration. It now depends on the new `Intent.Wolverine.Common` module, which seeds that registration, and appends its CQRS configuration statement to it at an explicit factory-extension `Order`. This removes the `lambdaBlock.Statements.Clear()` call that could silently discard another Wolverine module's contribution to the same lambda depending on execution order. This module no longer bundles its own copy of the `Intent.Wolverine.Common` assembly either, so the two can't disagree about which copy the module loader binds to. Generated output is unchanged.
