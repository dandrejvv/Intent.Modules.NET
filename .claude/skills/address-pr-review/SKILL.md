...
- **Invoked by hand** (`/address-pr-review …` typed in a chat) — there is no worktree and no session branch. You are in the user's own checkout, on their branch, quite possibly with their uncommitted work around you, and §5's commits would land there. Say that plainly and get confirmation before you commit anything.
...
- `headSha` — your working root's HEAD **after your last commit** (`git rev-parse HEAD`), not the SHA you started from.
